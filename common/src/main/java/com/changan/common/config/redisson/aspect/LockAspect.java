package com.changan.common.config.redisson.aspect;
...
public class LockAspect implements Ordered {
    ...
    @Around("@annotation(prop)")
    public Object handleLock(ProceedingJoinPoint pjp, Lock prop) throws Throwable {
        // 1.基于 SPEL 表达式解析锁的 name
        String name = getLockName(prop.name(), pjp);
        // 2.得到锁对象
        RLock lock = prop.lockType().getLock(redissonClient, name);
        // 3.尝试获取锁
        boolean success = prop.lockStrategy().tryLock(lock, prop);
        if (!success) {
            // 获取锁失败，结束
            return null;
        }
